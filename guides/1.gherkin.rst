Escribiendo Características - Lenguaje Gherkin
==============================================

Behat es una herramienta para probar el comportamiento de tu aplicación, descrito
es un lenguaje especial llamado Gherkin. Gherkin es un `Lenguaje Específico de Dominio, Legible por el negocio <http://martinfowler.com/bliki/BusinessReadableDSL.html>`_
creado específicamente para descripciones de comportamiento. Te da la habilidad de
ocultar detalles de la lógica de las pruebas de comportamiento.

Gherkin sirve dos propósitos: como documentación de tu proyecto y
como pruebas automatizadas. Behat tambien tiene una característica adicional: 
te habla de vuelta usando lenguaje real, humano, diciéndote que código debes escribir.

Si estás comenzando con Behat, salta a la :doc:`/quick_intro` primero, y luego
retorna aqui para aprender más acerca de Gherkin.

Sintáxis Gherkin
----------------

Al igual que YAML o Python, Gherkin es un lenguaje orientado a lineas, que usa la indentación para
definir la estructura. Los finales de lineas terminan las frases (llamadas pasos) y tanto espacios
como tabuladores pueden usarse para indentar (Sugerimos que uses espacios por portabilidad).
Finalmente, la mayoría de las lineas en Gherkin comienzan con una palabra especial:

.. code-block:: gherkin
    #language: es

    Característica: breve texto descriptivo de lo que se desea
      De manera de lograr un valor de negocio nombrado
      Como un actor explícito del sistema
      Yo quiero ganar alguna salida beneficiosa que apoye al objetivo
    
      Escenario: Alguna situación de negocio determinada
           Dado alguna precondición
              Y alguna otra precondición
         Cuando alguna acción por el actor
              Y alguna otra acción
              Y otra acción más
       Entonces alguna salida que puede ser probada es alcanzada
              Y algo más que podemos chequear sucede también

     Escenario: Una situación diferente
          ...

El parser divide la entrada en características, escenarios y pasos. Vamos a recorrer el ejemplo
de arriba:

1. ``Característica: breve texto descriptivo de lo que se desea`` comienza la
   característica y le dá un título. Aprende más acerca de características en la
   sección "`Características`_".

2. Behat no parsea las próximas 3 líneas de texto. (De manera de... Como un...Yo quiero...).
   Estas lineas simplemente proveen contexto a las personas leyendo tu característica, y describe
   el valor de negocio derivado de la inclusión de la característica en tu software.

3. ``Escenario: Alguna situación de negocio determinada`` comienza el escenario,
   y contiene una descripción del escenario. Aprende más acerca de los escenarios
   en la sección "`Escenarios`_".

4. Las próximas 7 lineas son los pasos del escenario, y cada uno coincide con una
   expresión regular definida en otro lugar. Aprende más de los pasos en la
   sección "`Pasos`_".

5. ``Escenario: Una situación diferente`` comienza el siguiente escenario, y asi en adelante.

Cuando estas ejecutando la característica, la porción del final de cada paso (luego de palabras
claves como ``Dado``, ``Y``, ``Cuando``, etc) coincide con una expresión regular, que ejecuta una llamada
a una función de PHP. Puedes leer más acerca de las coincidencias de pasos y la ejecución en :doc:`/guides/2.definitions`.
