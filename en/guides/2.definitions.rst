...
runned in Behat. But method existence itself is not enough for Behat to be able
...
.. image:: /images/example-snippets.png
   :align: center
...
.. image:: /images/example-successful.png
   :align: center
...
When Behat can nott find a matching definition, the step gets marked as
...
When a definition method throws ``Behat\Behat\Exception\Pending`` exception,
...
.. code-block:: php
    ...
    class FeatureContext extends BehatContext
    {
        ...
        public function someStepWithArgument($argument1)
        {
            throw new Pending('Do some string work');
        }

        /** @Given /^number step with (\d+)$/ */
        public function numberStepWith($argument1)
        {
            throw new Pending('Do some number work');
        }
    }

When you run your feature, you'll get 1 pending step, that are marked yellow:

.. image:: /images/example-pending.png
   :align: center
...
it's execution, the step is marked as **failed**. What you return from a
...
.. image:: /images/example-failed.png
   :align: center
...
When Behat find two or more definitions, that match single step, this step gets
marked as **ambiguos**.
...
.. image:: /images/example-ambiguous.png
   :align: center
...
In Behat you're not allowed to use a regexp more than once in a definitions, so
...
.. image:: /images/example-redundant.png
   :align: center
...
.. note::
    ...
    ``@Transform`` keyword and returns new value (wich will replace old one as
    definition argument).
...
step it's definition returns new ``Behat\Behat\Context\Step\Then`` instance.
...
.. note::
    ...
    Step, executed in chain will throw exception for any result type excluding
